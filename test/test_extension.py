import torch
from torch.testing._internal.common_utils import TestCase
from torch.testing._internal.optests import opcheck
import unittest
import extension_cpp
from torch import Tensor
from typing import Tuple
import torch.nn.functional as F
import torch.nn as nn
from extension_cpp.ops import reference_convrnn


class Testconvrnn_forward(TestCase):
    def sample_inputs(self, device, *, requires_grad=False):
        def make_tensor(*size):
            return torch.randn(size, device=device, requires_grad=requires_grad)

        def make_nondiff_tensor(*size):
            return torch.randn(size, device=device, requires_grad=False)
        
        def make_zero_tensor(*size):
            return torch.zeros(size, device=device, requires_grad=requires_grad)

        return [
            [make_tensor(8, 32, 128), make_tensor(1, 1, 3), make_tensor(8, 1, 128)]
            # [make_tensor(20), make_tensor(20), 3.14],
            # [make_tensor(20), make_nondiff_tensor(20), -123],
            # [make_nondiff_tensor(2, 3), make_tensor(2, 3), -0.3],
        ]

    def _test_correctness(self, device):
        samples = self.sample_inputs(device)
        for args in samples:
            for i, arg in enumerate(args):
                print(f"param {i} {arg.shape}")
            result = extension_cpp.ops.convrnn_interface(*args)
            expected = reference_convrnn(*args)
            torch.testing.assert_close(result, expected)

    # def test_correctness_cpu(self):
    #     self._test_correctness("cpu")

    def _test_gradients(self, device):
        param_names = [ 'x', 'kernel', 'hidden' ]
        samples = self.sample_inputs(device, requires_grad=True)
        for args in samples:
            diff_tensors = [a for a in args if isinstance(a, torch.Tensor) and a.requires_grad]
            out = extension_cpp.ops.convrnn_interface(*args)
            grad_out = torch.randn_like(out)
            result = torch.autograd.grad(out, diff_tensors, grad_out)

            out = reference_convrnn(*args)
            expected = torch.autograd.grad(out, diff_tensors, grad_out)

            for param_id in range(len(result)):
                print(f"grad for {param_names[param_id]} {param_id+1}/{len(result)}")
                # try:
                # print(result[param_id])
                # print(expected[param_id])
                torch.testing.assert_close(result[param_id], expected[param_id])
                # except AssertionError as e:
                    # print(f"grad failed {e.__traceback__}")
    # def test_gradients_cpu(self):
    #     self._test_gradients("cpu")

    def _opcheck(self, device):
        # Use opcheck to check for incorrect usage of operator registration APIs
        samples = self.sample_inputs(device, requires_grad=True)
        samples.extend(self.sample_inputs(device, requires_grad=False))
        for args in samples:
            opcheck(torch.ops.extension_cpp.convrnn_interface.default, args)

    def test_correctness_cuda(self):
        self._test_correctness("cuda")

    def test_gradients_cuda(self):
        self._test_gradients("cuda")

    # def test_opcheck_cuda(self):
    #     self._opcheck("cuda")



# class TestTorchCompileStreamSync(TestCase):
#     """Test for GitHub issue pytorch/pytorch#157363 - stream synchronization with torch.compile"""
    
#     @unittest.skipIf(not torch.cuda.is_available(), "requires cuda")
#     def test_compile_with_linear_layer(self):
#         """Test custom CUDA kernels with nn.Linear + torch.compile (the original failing case)"""
        
#         class Model(nn.Module):
#             def __init__(self, size):
#                 super().__init__()
#                 self.linear = nn.Linear(size, size, device="cuda", dtype=torch.float32)
            
#             def forward(self, x):
#                 return extension_cpp.ops.convrnn_forward(self.linear(x), self.linear(x), 0.0)
        
#         # Test sizes that previously failed
#         for size in [1000, 5000, 10000]:
#             with self.subTest(size=size):
#                 torch.manual_seed(42)
#                 model = Model(size)
#                 x = torch.randn((1, size), device="cuda", dtype=torch.float32)
                
#                 with torch.no_grad():
#                     expected = model(x)
#                     compiled_model = torch.compile(model, mode="reduce-overhead", fullgraph=True)
#                     actual = compiled_model(x)
                
#                 self.assertEqual(actual, expected)

#     @unittest.skipIf(not torch.cuda.is_available(), "requires cuda")
#     def test_compile_custom_only(self):
#         """Test custom operations alone with torch.compile"""
        
#         def model(x):
#             return extension_cpp.ops.convrnn_forward(x, x, 1.0)
        
#         for size in [1000, 5000, 10000]:
#             with self.subTest(size=size):
#                 torch.manual_seed(42)
#                 x = torch.randn((size,), device="cuda", dtype=torch.float32)
                
#                 with torch.no_grad():
#                     expected = model(x)
#                     compiled_model = torch.compile(model, mode="reduce-overhead", fullgraph=True)
#                     actual = compiled_model(x)
                
#                 self.assertEqual(actual, expected)


if __name__ == "__main__":
    unittest.main()
